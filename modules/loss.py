...
class CLIPLoss(nn.Module):
    ...
    def forward(self, sims, logit_scale):
        ...
        logits = sims * logit_scale   # 32,32
        
        t2v_log_sm = F.log_softmax(logits, dim=1)
        t2v_neg_ce = torch.diag(t2v_log_sm)     # 提取 t2v_log_sm 对角线上的元素
        t2v_loss = -t2v_neg_ce.mean()   # 首先，取 t2v_neg_ce 中的负值（乘以 -1），然后使用 .mean() 方法计算其平均值，得到一个标量张量作为最终的损失值
