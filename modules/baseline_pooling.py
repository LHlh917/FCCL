...
class BaselinePooling(nn.Module):
    ...
    def _topk_pooling(self, text_embeds, video_embeds):
        ...
        sims_topk = torch.topk(sims, self.k, dim=1)[1]   # 在第一维度进行在sims中取前self.k个的最大值
        ...
        video_embeds_topk = torch.gather(video_embeds, dim=1, index=sims_topk)   # torch.gather张量索引函数
    ...
    def _attention_pooling(self, text_embeds, video_embeds):
        ...
        video_embeds_pooled = torch.bmm(video_embeds, attention_weights)  # torch.bmm 矩阵相乘函数，用于计算两个张量的批矩阵乘法，用于3D张量
        return video_embeds_pooled.permute(0,2,1)
